...
public class Client implements ActionListener{         //JFrame class is used to creat frames in constructor  //ActionListener does an action when you click on it. it consist of an abstract methord in it so it needs to be override

    JTextField text;                    // Globally declared text field so we can use it in constructor and other functions 
    static JPanel a1;                          // Globally declared JPanel to access in another function (for appending "out" in it)
    ...
    Client(){                         // This is a constructor where the frame is run when main function calls obj and obj calls cunstructor
        ...
        ImageIcon i4 = new ImageIcon("icons/2.png");        // ImageIcon is used to bring image using its address
        ...
        JLabel name = new JLabel("Bunty");        // JLable is used to insert strings onto the panel, above it was used to insert images
        ...
        f.setLocation(800, 100);          // Sets the location on the screen where the frame will open (x-axis,y-axis)
    ...
    public void actionPerformed(ActionEvent ae){         // Used to override the Action Event function present in ActionListener function
        ...
        try{
            ...
            dout.writeUTF(out);                             // used to send the message to server
    ...
    public static void main(String[] args) {
        new Client();                               //annonomus object

        try{
            Socket s = new Socket("127.0.0.1", 6001);       //(local host, Server)    // Creating socket to connect with server in the Server.java   // A single server can connect to multiple socket
            ...
            while (true) {                      // While loop for infinatly sending and reciving messages
                ...
                vertical.add(Box.createVerticalStrut(15));        // a place to add the messages
